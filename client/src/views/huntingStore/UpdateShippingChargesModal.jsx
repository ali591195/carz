...
} from '../../actions/shippingChargesAction'
...
const UpdateShippingChargesModal = ({
  ...
  useEffect(() => {
    !error && setDeliveryCharges(storeShippingCharges)
  ...
  const validate = () => {
    ...
    if (deliveryCharges.length) {
      deliveryCharges.map((deliveryCharge, i) => {
        ...
        if (
          deliveryCharge.from !== '' &&
          deliveryCharge.from !== null &&
          Number(deliveryCharge.to) > Number(deliveryCharge.from)
        ) {
          isValid = false
  ...
  const onUpdate = (e) => {
    ...
    if (validate()) {
      ...
      updateShippingCharges(storeData, id)
    } else {
      setTimeout(() => {
        toast.error('Error occurs while Updating Shipping Charges.')
      }, 200)
    }
  }
                    ...
                    <Col sm={12} md={6} lg={3} style={{ margin: '10px 0px' }}>
                      <InputGroup>
                        ...
                        <Input
                          ...
                          name="from"
                          type="number"
                          // setDeliveryCharges((prevState) => [
                          //   ...prevState,
                          //   (prevState[index]['from'] = null),
                          // ])
...
UpdateShippingChargesModal.propTypes = {
  ...
  updateShippingCharges: PropTypes.func.isRequired,
